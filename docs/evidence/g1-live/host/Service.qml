...
Item {
  ...
  function onJournalWritten(text) {
    ...
    if (!ok) root.say("Grabbar could not save its recovery information")
  ...
  function onLine(data) {
    ...
    switch (msg.type) {
      ...
      case "notice":
        ...
        break
      case "menuRequest":
        // WindowMenu.qml is not built yet; the drawer is the text fallback.
        root.openDrawer()
        break
      case "event":
  ...
  function onWindow(msg) {
    ...
    if (kind === "title") {
      root.model = Model.updateTitle(root.model, win.title)
      root.publish()
      return
    }
  ...
  function openDrawer() {
    try {
      if (root.shell && typeof root.shell.summon === "function")
        root.shell.summon(root.pluginId, JSON.stringify({ view: "drawer" }))
    } catch (e) {}
  }
  ...
  IpcHandler {
    ...
    function reconcile(): string { root.journalConsumed = false; root.snapshotDone = false; root.send("snapshot", {}); return "requested" }
